-- migration_80c_multi_tier_instances.sql
-- Updates record_ticket_and_credit RPC to support multi-tier line items in a single PaymentIntent transaction:
-- 1. Inserts 1 ticket_orders summary row per PaymentIntent.
-- 2. Inserts N ticket_instances rows with individual ticket_id tier references and fresh unique QR codes.
-- 3. Inserts 1 single recipient_ledger_entries credit entry for the grand total.
