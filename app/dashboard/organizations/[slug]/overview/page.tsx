...
export default async function OrgOverviewPage({
  ...
  const { data: org } = await supabase
    .from("organizers")
    .select("id, name, follower_offset, events_offset, average_rating, review_count")
    .eq("slug", slug)
    .maybeSingle();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Recent Events — divide-y rows separate items, no outer box. StatCards above are KPI entities and keep their boundaries. */}
      ...
      <section className="border-t border-zinc-200 pt-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link href={`/organizations/${slug}`} target="_blank" className="rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm font-bold text-zinc-700 transition hover:bg-zinc-50">View Public Profile ↗</Link>
        </div>
      </section>
    </div>
  );
