...
// public events — powers the Location autocomplete's suggestion list.
const getCachedEventCities = unstable_cache(
  async () => {
    const adminClient = createSupabaseAdmin();
...
// "force-dynamic"` below makes every `fetch()` in this route equivalent to
// `{ cache: "no-store" }` (see node_modules/next/dist/docs/01-app/02-guides/
// caching-without-cache-components.md) — that silently defeats the
// `next: { revalidate: 300 }` set inside searchTicketmaster/searchSeatGeek.
// unstable_cache is a separate caching layer unaffected by that override
// (same reason the three caches above exist), keyed automatically by its
// arguments, so each distinct location/query/category/date combination gets
// its own 20-minute-cached entry regardless of how many visitors hit it.
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://www.fund4agoodcause.com/events",
  },
  openGraph: {
    title: "Events — Aldriva",
    description: "Browse and buy tickets for events near you.",
...
export default async function EventsPage({
  ...
  // Category chip grid — shared with the homepage (lib/category-chips).
  ...
  // searched for (the `location` filter) first, and only falls back to the
  // visitor's IP/edge-detected city when nothing has been searched.
  ...
  //
  // Blended on every /events view now, not just active searches — but only
  // ever rendered on page 1 (see the ExternalEventsCarousel section below),
  // so the fetch is scoped to page 1 too rather than burning API quota on
  // pages whose results are never shown. No `location` (the default,
  // unfiltered view) is a deliberate nationwide query, not a skipped one —
  // Ticketmaster falls back to countryCode: "US" and SeatGeek omits
  ...
  const buildQuery = (extra: Record<string, string>) => {
    ...
    if (activeWhen === "custom") {
      if (customFrom) params.set("from", customFrom);
      if (customTo) params.set("to", customTo);
    } else if (activeWhen !== "all") {
      params.set("when", activeWhen);
    }
  ...
  // 5. Trending events (step 5) — ranked by all-time ticket sales, topped up
  ...
  return (
    <main className="min-h-screen bg-zinc-50 text-zinc-950 pb-16">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-8 pt-8 sm:px-6 sm:pb-10 sm:pt-10 lg:px-8">
        ...
        {/* ── Results grid — filter/sort controls removed. Renders the default
            getEventList query; default sort is date_asc — soonest event date
            first (id as tie-breaker). ── */}
        {!hideEventsSidebar &&
