...
test("EventSubNav role matrix matches event-team access expectations", () => {
  ...
  assert.deepEqual(
    ...
    ["operations", "checkins", "scan", "guests", "ticket-design", "invitation-design", "seating", "team"]
  );
  assert.equal(canShowEventPublicPage("event_manager", "public-slug"), false);

  assert.deepEqual(
    getEventSubNavTabs("evt_1", "owner").map((tab) => tab.id),
    ["operations", "checkins", "scan", "guests", "ticket-design", "invitation-design", "seating", "team", "edit"]
  );
