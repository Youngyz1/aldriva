...
test("H4: authenticated non-admins are forbidden (admin-only scaffold)", async () => {
  const previousEnv = process.env.NODE_ENV;
  process.env.NODE_ENV = "development";
  try {
    getUserCalls = 0;
...
test("H4: admins pass the gate", async () => {
  const previousEnv = process.env.NODE_ENV;
  process.env.NODE_ENV = "development";
  try {
    getUserCalls = 0;
    ...
    const route = loadRoute();
    const res = await route.GET();
    // createArticle mock returns {} (no data) → 500 "Failed to create",
    // which proves the request passed all three gates (env, auth, admin).
